...
//TODO: (EW) reduce duplicate code from DraggableListAdapter
/**
 ...
 */
public class DraggableGroupedListAdapter<TGroup, TItem> extends BaseAdapter {
    ...
    private final List<GroupData> mGroups = new ArrayList<>();
    ...
    private final ListItemBuilder<TGroup, TItem> mListItemBuilder;
    ...
    private GroupedIndex getGroupedIndex(int flatListIndex) {
        if (mAreGroupsExpanded) {
            ...
            while (groupIndex + 1 < mGroups.size()
                    && itemIndex >= mGroups.get(groupIndex).mItems.size()) {
                itemIndex -= mGroups.get(groupIndex).mItems.size() + 1;
                groupIndex++;
            }
    ...
    public Object getItem(int position) {
        ...
        if (index.mItemIndex == NO_FIELD_INDEX) {
            return mGroups.get(index.mGroupIndex).mGroup;
        }
        return mGroups.get(index.mGroupIndex).mItems.get(index.mItemIndex);
    }
    ...
    public void addItem(int groupIndex, TItem row) {
        mGroups.get(groupIndex).mItems.add(row);
        notifyDataSetChanged();
    }
    ...
    public TGroup getGroup(int groupIndex) {
        return mGroups.get(groupIndex).mGroup;
    }

    public int getItemCount(int groupIndex) {
        return mGroups.get(groupIndex).mItems.size();
    }

    public TItem getItem(int groupIndex, int itemIndex) {
        return mGroups.get(groupIndex).mItems.get(itemIndex);
    }
    ...
    public void move(int fromIndex, int toIndex) {
        ...
        if (mAreGroupsExpanded) {
            ...
            GroupData fromGroup = mGroups.get(fromGroupedIndex.mGroupIndex);
            ...
            if (toGroupedIndex.mItemIndex == NO_FIELD_INDEX) {
                if (toGroupedIndex.mGroupIndex > 0 && fromIndex > toIndex) {
                    ...
                    toItemIndex = mGroups.get(toGroupIndex).mItems.size();
                } else {
            ...
            mGroups.get(toGroupIndex).mItems.add(toItemIndex, item);
        } else {
            GroupData group = mGroups.get(fromIndex);
            mGroups.remove(fromIndex);
            mGroups.add(toIndex, group);
        }
        notifyDataSetChanged();
    }
    ...
    private void updateView(View view, int index) {
        ...
        GroupData group = mGroups.get(groupedIndex.mGroupIndex);
    ...
    private void updateViewsForDrag(int currentDropHoverIndex) {
        ...
        int count = getCount();
        if (currentDropHoverIndex < 0 || currentDropHoverIndex >= count) {
    ...
    private synchronized void startScrollTimer(int scrollDirection) {
        ...
        mScrollTimer.scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                ...
                if (mCurrentScroll > 0) {
                    View view = getItemView(getCount() - 1);
