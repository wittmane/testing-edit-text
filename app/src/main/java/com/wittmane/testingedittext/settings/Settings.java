...
public class Settings implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public static boolean getTestFieldAllowSettingSelection(int fieldIndex) {
        return getInstance().mTestFields.get(fieldIndex).mAllowSettingSelection;
    }
    ...
    private static void convertPrefs(final SharedPreferenceManager prefs) {
        String[] appLevelBooleanPrefKeys = new String[] {
                PREF_MODIFY_COMMITTED_TEXT_PREFIX,
                PREF_MODIFY_COMPOSED_TEXT_PREFIX,
                PREF_MODIFY_COMPOSED_CHANGES_ONLY_PREFIX,
                PREF_CONSIDER_COMPOSED_CHANGES_FROM_END_PREFIX,
        ...
        for (String prefKey : appLevelBooleanPrefKeys) {
            if (prefs.contains(prefKey)) {
                prefs.setBoolean(prefKey + BASE_SUFFIX, prefs.getBoolean(prefKey, false));
                prefs.remove(prefKey);
            }
        }
        String[] appLevelStringArrayPrefKeys = new String[] {
                PREF_RESTRICT_SPECIFIC_PREFIX,
                PREF_TRANSLATE_SPECIFIC_PREFIX,
        };
        for (String prefKey : appLevelStringArrayPrefKeys) {
            if (prefs.contains(prefKey)) {
                prefs.setStringArray(prefKey + BASE_SUFFIX, prefs.getStringArray(prefKey, null));
                prefs.remove(prefKey);
            }
        }
        String[] appLevelStringPrefKeys = new String[] {
                PREF_RESTRICT_RANGE_PREFIX,
        };
        for (String prefKey : appLevelStringPrefKeys) {
            if (prefs.contains(prefKey)) {
                prefs.setString(prefKey + BASE_SUFFIX, prefs.getString(prefKey, null));
                prefs.remove(prefKey);
            }
        }
        String[] appLevelIntPrefKeys = new String[] {
                PREF_SHIFT_CODEPOINT_PREFIX,
                PREF_LIMIT_EXTRACT_MONITOR_TEXT_PREFIX,
                PREF_LIMIT_RETURNED_TEXT_PREFIX,
                PREF_UPDATE_DELAY_PREFIX,
        ...
        for (String prefKey : appLevelIntPrefKeys) {
            if (prefs.contains(prefKey)) {
                prefs.setInt(prefKey + BASE_SUFFIX, prefs.getInt(prefKey, 0));
                prefs.remove(prefKey);
            }
        }
        for (String prefKey : appLevelIntPrefKeys) {
        ...
        final String[] testFieldStringPrefKeyPrefixes = new String[]{
                PREF_INPUT_TYPE_CLASS_PREFIX,
                PREF_INPUT_TYPE_TEXT_VARIATION_PREFIX,
                PREF_INPUT_TYPE_NUMBER_VARIATION_PREFIX,
                PREF_INPUT_TYPE_DATETIME_VARIATION_PREFIX,
        ...
        final String[] testFieldBooleanPrefKeyPrefixes = new String[]{
                PREF_INPUT_TYPE_TEXT_FLAG_AUTO_COMPLETE_PREFIX,
                PREF_INPUT_TYPE_TEXT_FLAG_AUTO_CORRECT_PREFIX,
                PREF_INPUT_TYPE_TEXT_FLAG_NO_SUGGESTIONS_PREFIX,
                PREF_INPUT_TYPE_NUMBER_FLAG_SIGNED_PREFIX,
        ...
        final String[] testFieldIntPrefKeyPrefixes = new String[]{
                PREF_IME_ACTION_ID_PREFIX,
                PREF_MAX_LENGTH_PREFIX,
        };
        final String[] testFieldStringArrayPrefKeyPrefixes = new String[]{
                PREF_TEXT_LOCALES_PREFIX,
                PREF_IME_HINT_LOCALES_PREFIX,
        };
        final String[] testFieldCharSequencePrefKeyPrefixes = new String[]{
                PREF_IME_DEFAULT_TEXT_PREFIX,
                PREF_IME_HINT_TEXT_PREFIX
        };
        for (int fieldId : readTestFieldIds(prefs)) {
            for (String prefKeyPrefix : testFieldStringPrefKeyPrefixes) {
                String oldPrefKey = getOldTestFieldPrefKey(prefKeyPrefix, fieldId);
                String newPrefKey = prefKeyPrefix + FIELD_INFIX + fieldId;
                if (prefs.contains(oldPrefKey)) {
                    prefs.setString(newPrefKey, prefs.getString(oldPrefKey, null));
                    prefs.remove(oldPrefKey);
                }
            }
            for (String prefKeyPrefix : testFieldBooleanPrefKeyPrefixes) {
                String oldPrefKey = getOldTestFieldPrefKey(prefKeyPrefix, fieldId);
                String newPrefKey = prefKeyPrefix + FIELD_INFIX + fieldId;
                if (prefs.contains(oldPrefKey)) {
                    prefs.setBoolean(newPrefKey, prefs.getBoolean(oldPrefKey, false));
                    prefs.remove(oldPrefKey);
                }
            }
            for (String prefKeyPrefix : testFieldIntPrefKeyPrefixes) {
                String oldPrefKey = getOldTestFieldPrefKey(prefKeyPrefix, fieldId);
                String newPrefKey = prefKeyPrefix + FIELD_INFIX + fieldId;
                if (prefs.contains(oldPrefKey)) {
                    prefs.setInt(newPrefKey, prefs.getInt(oldPrefKey, 0));
                    prefs.remove(oldPrefKey);
                }
            }
            for (String prefKeyPrefix : testFieldStringArrayPrefKeyPrefixes) {
                String oldPrefKey = getOldTestFieldPrefKey(prefKeyPrefix, fieldId);
                String newPrefKey = prefKeyPrefix + FIELD_INFIX + fieldId;
                if (prefs.contains(oldPrefKey)) {
                    prefs.setStringArray(newPrefKey, prefs.getStringArray(oldPrefKey, null));
                    prefs.remove(oldPrefKey);
                }
            }
            for (String prefKeyPrefix : testFieldCharSequencePrefKeyPrefixes) {
                String oldPrefKey = getOldTestFieldPrefKey(prefKeyPrefix, fieldId);
                String newPrefKey = prefKeyPrefix + FIELD_INFIX + fieldId;
                if (prefs.contains(oldPrefKey)) {
                    prefs.setCharSequence(newPrefKey, prefs.getCharSequence(oldPrefKey, null));
                    prefs.remove(oldPrefKey);
                }
            }
        }
    ...
    private static String getOldTestFieldPrefKey(String newPrefKeyPrefix, int fieldId) {
        if (!newPrefKeyPrefix.startsWith("pref_key_")) {
            throw new RuntimeException("unexpected preference key prefix: " + newPrefKeyPrefix);
        }
        return "pref_key_test_field_" + newPrefKeyPrefix.substring("pref_key_".length()) + "_" + fieldId;
    }
}
