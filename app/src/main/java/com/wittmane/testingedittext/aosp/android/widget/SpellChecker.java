...
import com.wittmane.testingedittext.aosp.com.android.internal.util.GrowingArrayUtils;
...
public class SpellChecker implements SpellCheckerSessionListener {
    ...
    // Shared by all SpellParsers. Cannot be shared with EditText since it may be used
        ...
        for (int i = 0; i < mLength; i++) {
            ...
            if (start >= 0 && end > start && (forceCheckWhenEditingWord || isNotEditing)) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    textInfo = new TextInfo(editable, start, end, mCookie, mIds[i]);
                } else {
                    final String word;
                    if (editable instanceof SpannableStringBuilder) {
                        char[] buf = new char[end - start];
                        editable.getChars(start, end, buf, 0);
                        word = new String(buf);
                    } else {
                        word = editable.subSequence(start, end).toString();
                    }
                    textInfo = new TextInfo(word, mCookie, mIds[i]);
                }
    ...
    private class SpellParser {
        ...
        public void parse() {
            ...
            RangeWrapper<Integer> sentenceBoundary = detectSentenceBoundary(editable,
    ...
    private RangeWrapper<Integer> detectSentenceBoundary(CharSequence sequence,
        ...
        return new RangeWrapper<>(sentenceStart, Math.max(sentenceStart, sentenceEnd));
    }
    ...
    public static boolean haveWordBoundariesChanged(final Editable editable, final int start,
                                                    final int end, final int spanStart,
                                                    final int spanEnd) {
    ...
    private static final class RangeWrapper<T extends Comparable<? super T>> {
        ...
        public RangeWrapper(final T lower, final T upper) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                mRange = new Range<>(lower, upper);
                mLower = null;
                mUpper = null;
            } else {
                mRange = null;
